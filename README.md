...
service-routing-configmap.yaml   etraffica-service-routing  — 24 *_SERVICE_URL → the -nest Services (gateway only; GENERATED)
secrets.yaml.example             etraffica-secrets template — copy to secrets.yaml, fill in
api-gateway.yaml                 Deployment + Service (port 4300); embeds legacy-parity (/api/v1/legacy)
services-nest/<name>-nest.yaml   the backend: one NestJS+TypeORM Deployment + ClusterIP Service per service (GENERATED by tools/generate-nest-manifests.js)
workers.yaml                     5 outbox/notification/payment/workflow/media worker Deployments
workers-nest.yaml                the rmq-relay worker (drains domain events → owning collections)
...
# 4. Backend services (NestJS twins), gateway, workers, frontends
kubectl apply -f services-nest/
kubectl apply -f api-gateway.yaml
kubectl apply -f workers.yaml -f workers-nest.yaml
...
```bash
kubectl apply -f services-nest/ -f api-gateway.yaml -f workers.yaml -f workers-nest.yaml -f frontends/
